...
export default {
  ...
  methods: {
    async initLetData(selectedPaper) {
      ...
      // 3，	案发时间：检查笔录中时间
      let string3 = `    三、案发时间：${let1DataPaperContent.cellIdx1 || 'XXXX年XX月XX日'}。\r\n`;
      // 4，	主要违法事实：检查笔录中时间+机构名称+登录人员名称+“等监察员按照监察执法计划，依法对”+煤矿名称+“进行现场检查时,发现该矿”+隐患描述
      let string4 = `    四、主要违法事实：${let1DataPaperContent.cellIdx1 || 'XX'}${this.$store.state.user.userGroupName || 'XX'}${this.$store.state.user.userName || 'XX'}等监察员按照监察执法计划，依法对${corp.corpName || 'XX'}进行现场检查时,发现该矿${dangerObject.dangerString || 'XX'}。\r\n`;
      // 5，	立案调查及审理情况：
      ...
      let string5 = `    五、立案调查及审理情况：${let4DataPaperContent.cellIdx6 || 'XXXX'}年${let4DataPaperContent.cellIdx7 || 'XX'}月${let4DataPaperContent.cellIdx8 || 'XX'}日经${this.$store.state.user.userGroupName || 'XX'}负责人批准，决定对该涉嫌违法行为进行立案调查，并指定由、两名执法人员承办。案件承办人对进行调查取证，制作了调查取证笔录，收集了该矿违法违规行为的其他证据材料。`;
      ...
      string5 += `${let6Date[0]}年${let6Date[1]}月${let6Date[2]}日案件承办人分别向${corp.corpName}及其负责人XXX下达了《行政处罚告知书》（${let6DataPaperContent.cellIdx0}矿安监${let6DataPaperContent.cellIdx1}告〔${let6DataPaperContent.cellIdx2}〕${let6DataPaperContent.cellIdx3}号）。该矿负责人签收文书后，未进行陈述、申辩，未要求举行听证。`;
      // 行政处罚决定书8制作时间、文书编号
      ...
      let let206PaperNum = `${let8DataPaperContent.cellIdx0}矿安监${let8DataPaperContent.cellIdx1}罚〔${let8DataPaperContent.cellIdx2}〕${let8DataPaperContent.cellIdx3}号`;
