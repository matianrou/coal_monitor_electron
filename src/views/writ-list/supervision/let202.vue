...
export default {
  ...
  methods: {
    async initLetData (selectedPaper) {
      ...
      let cellIdx21String = `我们是${this.$store.state.curCase.groupName}监管员，这是我们的执法证件（出示行政执法证件），现就你${corp.corpName}涉嫌${dangerObject.dangerString}违法违规案向你进行调查取证，你有配合调查、如实回答问题的义务，也享有拒绝回答与调查取证无关问题的权利，但不得做虚假陈述和伪证，否则，将负相应的法律责任，你听清楚了吗？`
      await db.close();
      this.letData = {
        cellIdx0: null, // 年
        cellIdx1: null, // 月
